"""
GAN oversampling v4 — anti-collapse training.

Current best: F1=0.7483  Paper target: 0.7981  Need: ≥0.7582 (<5% gap)

v3 hit F1=0.7340 due to mode collapse in fold 5.
Fixes:
  1. Gradient clipping (max_norm=1.0) on both D and G — prevents weight explosion
  2. n-critic=2: train D twice per G step — keeps D from being overtaken
  3. Label smoothing=True (kept from v3)
"""
...
CONFIGS = [
    dict(epochs=100, noise_dim=32, n_critic=2),
    dict(epochs=100, noise_dim=64, n_critic=2),
]
HIDDEN   = 128
LR       = 5e-5
...
def train_gan(model, X_tr, y_tr, epochs, lr, noise_dim, n_critic=2, batch=256):
    n_pos = (y_tr == 1).sum()
    n_neg = (y_tr == 0).sum()
    ...
    for epoch in range(epochs):
        for xb, yb in loader:
            ...
            for _ in range(n_critic):
                opt_d.zero_grad()
                yb_smooth = yb * 0.9          # label smoothing
                d_loss = (criterion(model.discriminator(xb).squeeze(-1), yb_smooth) +
                          0.5 * criterion_g(
...
def run_cv(X, y, sampler, epochs, noise_dim, n_critic):
    ...
    for fold_i, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        train_gan(model, X_bal, y_bal, epochs=epochs, lr=LR,
                  noise_dim=noise_dim, n_critic=n_critic)
    ...
    return {
        ...
        "epochs": epochs, "noise_dim": noise_dim, "n_critic": n_critic,
    }
...
def main():
    ...
    print(f"Baseline F1=0.7483 | v3 best F1=0.7340 (collapsed fold 5)")
    print(f"Fix: grad_clip={MAX_NORM}, n_critic=2, label_smooth=0.9\n")
    ...
    for cfg in CONFIGS:
        t0 = time.time()
        label = f"epochs={cfg['epochs']} noise={cfg['noise_dim']} n_critic={cfg['n_critic']}"
        print(f"\n{'='*60}\n{label}")
    ...
    out = os.path.join(os.path.dirname(__file__), "..", "results", "tune_gan_over_v4.csv")
    ...
    print(f"BEST:  epochs={best.epochs}  noise_dim={best.noise_dim}  "
          f"n_critic={best.n_critic}  hidden={HIDDEN}  lr={LR}")
