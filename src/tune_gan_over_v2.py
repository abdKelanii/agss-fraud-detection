"""
GAN oversampling tuning v2 — creditcard AGSS.

Current best: F1=0.7483  Paper target: 0.7981  Need: ≥0.7582 (<5% gap)

Improvements over v1:
  1. More epochs (150, 200, 300)
  2. Label smoothing (real labels → 0.9 instead of 1.0)
  3. Larger noise_dim (64)
  4. BatchNorm in generator for stable training
"""
...
GRID = {
    "epochs":       [150, 200, 300],
    "noise_dim":    [32, 64],
    "label_smooth": [False, True],
}
# Fixed best hyperparams from v1
HIDDEN = 128
LR     = 5e-5


class FraudGANv2(nn.Module):
    """GAN with BatchNorm in generator for improved training stability."""
...
def train_gan(model, X_tr, y_tr, epochs, lr, noise_dim, label_smooth=False, batch=256):
    n_pos = (y_tr == 1).sum()
    n_neg = (y_tr == 0).sum()
    ...
    for _ in range(epochs):
        for xb, yb in loader:
            ...
            opt_d.zero_grad()
            # Use smoothed real labels for discriminator
...
def main():
    ...
    combos  = list(itertools.product(
        GRID["epochs"], GRID["noise_dim"], GRID["label_smooth"]
    ))
    print(f"Running {len(combos)} configs (hidden={HIDDEN}, lr={LR}) …\n")

    rows = []
    for epochs, noise_dim, label_smooth in combos:
        t0 = time.time()
        label = f"epochs={epochs} noise={noise_dim} smooth={label_smooth}"
        print(f"{label} … ", end="", flush=True)
        res = run_cv(X, y, sampler, epochs=epochs, noise_dim=noise_dim,
                     label_smooth=label_smooth)
        elapsed = time.time() - t0
        res["runtime_s"] = elapsed
        rows.append(res)
        gap = 0.7981 - res["f1"]
        flag = "✅ <5%" if gap < 0.03991 else f"  ({gap:.4f} gap)"
    ...
    print(f"\n{'='*60}")
    print(f"BEST CONFIG:")
    print(f"  epochs={best.epochs}  noise_dim={best.noise_dim}  "
          f"label_smooth={best.label_smooth}  hidden={HIDDEN}  lr={LR}")
    print(f"  F1={best.f1:.4f}  Prec={best.precision:.4f}  "
          f"Rec={best.recall:.4f}  ROC={best.roc_auc:.4f}")
    print(f"  threshold={best.threshold}")
    gap = 0.7981 - best.f1
    print(f"  Gap vs paper: {gap:.4f} ({100*gap/0.7981:.1f}%)")
    print(f"  {'✅ GOAL MET (<5% gap)' if gap < 0.03991 else '❌ Still above 5% gap'}")
    print(f"\nSaved: {out}")
