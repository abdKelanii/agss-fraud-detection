"""
GAN oversampling v6 — fold-specific seed only (no clip overhead on MPS).

Current best: F1=0.7483  Paper target: 0.7981  Need: ≥0.7582 (<5% gap)

Root cause of fold-5 collapse: deterministic bad init with sequential seeds.
Fix: torch.manual_seed(RANDOM_STATE * (fold_i + 1)) per fold — zero overhead.

grad_clip was removed: clip_grad_norm_ forces MPS device sync per batch
(~10ms × 177K batches = ~30 min per fold). Not viable on MPS.

3 configs: noise_dim=32 (baseline comparison), 64, and 32 with more epochs.
"""
...
CONFIGS = [
    dict(epochs=100, noise_dim=32),   # baseline reproduction with fold seeds
    dict(epochs=100, noise_dim=64),   # larger noise space
    dict(epochs=120, noise_dim=32),   # a bit more training
]
HIDDEN = 128
LR     = 5e-5
...
def run_cv(X, y, sampler, epochs, noise_dim):
    ...
    for fold_i, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
        # Per-fold seed: each fold gets a different but deterministic init
        ...
        print(f"  fold {fold_i+1}/{N_FOLDS}  F1@0.4={f1_04:.4f}  ROC={roc:.4f}  "
              f"seed={RANDOM_STATE*(fold_i+1)}", flush=True)
...
def main():
    ...
    print(f"Baseline F1=0.7483 | Fix: per-fold seeds\n")
    ...
    out = os.path.join(os.path.dirname(__file__), "..", "results", "tune_gan_over_v6.csv")
